...
`hashed` 또는 `vertex` 를 받고 기본값이 없다. 이는 양쪽 기본값이 달라서 생기던 불일치를 없앤
것이며, 두 프로세스가 서로의 값을 비교하지는 않는다. 인제스트에 `hashed`, 서버에 `vertex` 를
명시하면 둘 다 오류 없이 실행되고 768차원이 같아 이후 단계도 알아채지 못한다. 이미 저장된
인덱스가 어느 임베더로 만들어졌는지도 아직 증명하지 못한다. 과거에 보고된
